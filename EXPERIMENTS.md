...
### 観察結果②
同じallkeys-lru・同じメモリ上限でも、全員を単一キーに入れると上限のたびに全損する一方、read/writeのまとまりでキーを分割すると、LRUは冷たいシャードから最小限に追い出し、ホットなデータを守れた(old:0/old:1が0になる一方hotは生存)。ゆえにエビクション時に「何が生き残るか」はポリシーだけでなくキーの切り方が決める。
